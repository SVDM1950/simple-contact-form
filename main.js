...
class SimpleContactForm {
    ...
    modifyMenuStructure(rendererInstance, output, ...params) {
        ...
        // this.API.createFile(`[ROOT-FILES]/${uuid}.modifyMenuStructure.output.json`, JSON.stringify(output), this);
        // this.API.createFile(`[ROOT-FILES]/${uuid}.modifyMenuStructure.params.json`, JSON.stringify(params), this);
    ...
    generateContactForm(rendererInstance, output, globalContext, context) {
        ...
        globalContext.plugins.simpleContactForm.config.recaptcha = this.config.recaptchaSiteKey && !this.rendererInstance.previewMode;

        context.title = `${globalContext.website.name} - ${translations.menu.label}`;
        context.contact = {
            title: `${translations.menu.label}`,
            featuredImage: false,
            text: globalContext.plugins.simpleContactForm.config.contactContent,
        }

        if(globalContext.plugins.simpleContactForm.config.contactFeaturedImage !== '') {

            let imageUrl = globalContext.plugins.simpleContactForm.config.contactFeaturedImage;
            let imageData = {
    ...
    getFeaturedImages(imageFilename, featuredImageData) {
        ...
        if (contactImage && contactImage.url) {
            ...
            featuredImage.createResponsiveImages(imagePath);
